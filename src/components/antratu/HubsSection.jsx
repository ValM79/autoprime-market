  ...
  return (
    <section className="bg-secondary py-12 md:py-16">
      <div className="max-w-7xl mx-auto px-4">
        <h2 className="text-2xl md:text-3xl font-bold text-center text-foreground mb-2">{t(lang, 'hubs_title')}</h2>
        <p className="text-center text-muted-foreground mb-8">{t(lang, 'hubs_subtitle')}</p>

        <div className="flex items-center gap-3">
          <button onClick={prev} className="flex-shrink-0 w-10 h-10 bg-white shadow-lg rounded-full flex items-center justify-center hover:bg-card transition-colors">
          ...
          <div className="flex-1 grid grid-cols-3 gap-4">
            {visible.map((hub, idx) => (
              ...
              >
                {hub.image && (
                  <img src={hub.image} alt={hub.labelKey} className="absolute inset-0 w-full h-full object-cover opacity-25 group-hover:opacity-40 transition-opacity" />
                )}
          ...
          <button onClick={next} className="flex-shrink-0 w-10 h-10 bg-white shadow-lg rounded-full flex items-center justify-center hover:bg-card transition-colors">
